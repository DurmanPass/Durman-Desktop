...
export class CategoryService {
    ...
    async createCategory(name: string): Promise<{ id: string; message: string }> {
        ...
        return withTokenRefresh(this.http, this.refreshTokenService, headers =>
            this.http.post(ApiRoutes.CATEGORY.CREATE_CATEGORY, body, { headers }).pipe(
                map((response: any) => {
                    if (response.id && response.message) {
                        ToastService.success('Категория успешно создана!');
                        return response as { id: string; message: string };
    ...
    async getAllCategoryUser(): Promise<Category[]> {
        return withTokenRefresh(this.http, this.refreshTokenService, headers =>
            this.http.get(ApiRoutes.CATEGORY.GET_ALL_CATEGORY_USER, { headers }).pipe(
                map((response: any) => {
                    if (Array.isArray(response)) {
                        return response as Category[];
                    } else {
    ...
    async getCategoryById(categoryId: string): Promise<Category> {
        return withTokenRefresh(this.http, this.refreshTokenService, headers =>
            this.http.get(ApiRoutes.CATEGORY.GET_CATEGORY_BY_ID(categoryId), { headers }).pipe(
                map((response: any) => {
                    if (response.id) {
                        return response as Category;
                    } else {
    ...
    async updateCategory(category: Category): Promise<void> {
        return withTokenRefresh(this.http, this.refreshTokenService, headers =>
            this.http.put(ApiRoutes.CATEGORY.UPDATE_CATEGORY(category.id), { name: category.name }, { headers }).pipe(
                map(() => {
                    ToastService.success('Категория успешно обновлена!');
                })
            )
    ...
    async deleteCategory(categoryId: string): Promise<void> {
        return withTokenRefresh(this.http, this.refreshTokenService, headers =>
            this.http.delete(ApiRoutes.CATEGORY.DELETE_CATEGORY(categoryId), { headers }).pipe(
                map(() => {
                    ToastService.success('Категория успешно удалена!');
                })
            )
